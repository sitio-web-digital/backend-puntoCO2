...
import { listServiciosPaginado, calcularMargen } from "@/server/servicios/service";
...
export default async function ServiciosPage({ searchParams }: { searchParams: Promise<{ serviciosPage?: string }> }) {
  const user = await getCurrentUser();
  if (!user || !user.tenantId) redirect("/login");
  const actor: TenantActor = { tenantId: user.tenantId, usuarioId: user.usuarioId };

  const { serviciosPage } = await searchParams;
  const {
    items: servicios,
  ...
  } = await listServiciosPaginado(actor, { page: serviciosPage ? Number(serviciosPage) : 1 });
  ...
  return (
    <div className="stack">
      <div className="stack" style={{ gap: 5 }}>
        ...
        <p className="muted">
          {totalServicios} servicio{totalServicios === 1 ? "" : "s"} en el catálogo
        </p>
      </div>
